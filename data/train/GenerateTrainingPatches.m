...
Datasets = {'Flickr1024'};  % Original datasets for training data generation
scale = 4;       % down-scaling factor
...
for idx_dataset = 1 : length(Datasets)
    ...
    for idx_file = 1 : 2 : length(img_list)                
        ...
        %% extract patches of size 128*128 with stride 20
        
        for x_lr = 4 : 20 : size(img_lr_0,1) - 132
            for y_lr = 4 : 20 : size(img_lr_0,2) - 132
                x_hr = (x_lr - 1) * scale + 1;
                y_hr = (y_lr - 1) * scale + 1;
                hr_patch_0 = img_hr_0(x_hr : (x_lr+127)*scale, y_hr : (y_lr+127)*scale, :);
                hr_patch_1 = img_hr_1(x_hr : (x_lr+127)*scale, y_hr : (y_lr+127)*scale, :);
                lr_patch_0 = img_lr_0(x_lr : x_lr+127, y_lr : y_lr+127, :);
                lr_patch_1 = img_lr_1(x_lr : x_lr+127, y_lr : y_lr+127, :);
